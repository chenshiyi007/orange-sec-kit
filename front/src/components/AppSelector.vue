<template>
  <a-select
    ...
    :options="options"
    :placeholder="t('components.appSelector.enterAppName')"
...
const getApps = async (name: string) => {
  ...
  try {
    ...
    if (res.status !== 200) {
      Notification.warning(t('common.serviceError'))
      return
    }
    if (res.data?.code === 0) {
      ...
      Notification.warning(res.data?.message || t('components.appSelector.requestFailed'))
    }
  } catch {
    Notification.error(t('components.appSelector.requestError'))
  } finally {
    loading.value = false
  }
}
